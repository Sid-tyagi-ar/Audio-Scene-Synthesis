...
class Encoder(nn.Module):
    def __init__(self, input_channels, embedding_size):
        ...
        self.embedding = nn.Linear(256 * 512, embedding_size)
...
class CLAP(nn.Module):
    def __init__(self,
        ...
        self.auto_enc = Encoder(1, 512)
        self.auto_enc.load_state_dict(torch.load("audioldm_train/modules/MSCLAP/msclap/models/encoder_model.pth"))
...
#         model_fp = hf_hub_download("microsoft/msclap", "CLAP_weights_2023.pth")
#         model_state_dict = torch.load(model_fp, map_location=torch.device("cuda"))['model']   
#         text_encoder = TextEncoder(
#                 1024, 'gpt2', 768
#             ).load_state_dict(model_state_dict, strict=True)
#         tokenize = AutoTokenizer.from_pretrained('gpt2')
#         tokenize.pad_token = tokenize.eos_token  # Set the pad_token to the eos_token
#         encoded_texts = tokenize("This is working", padding=True, truncation=True, return_tensors="pt").to(device = "cuda")
#         print(text_encoder(encoded_texts))
