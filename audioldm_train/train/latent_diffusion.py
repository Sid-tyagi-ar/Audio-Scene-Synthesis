...
def main(configs, config_yaml_path, exp_group_name, exp_name, perform_validation):
    ...
    if 'desktop.ini' in os.listdir(checkpoint_path):
        os.remove(os.path.join(checkpoint_path, 'desktop.ini'))
    ...
    # def print_model_summary(model, input_sizes=None):
    #     """Prints a comprehensive summary of a PyTorch model, handling multiple inputs.

    #     Args:
    #         model (torch.nn.Module): The PyTorch model to summarize.
    #         input_sizes (list of tuples, optional): A list of tuples specifying the
    #             size of each input tensor. The order of the tuples should match
    #             the order of the model's inputs. If not provided, the summary
    #             will attempt to infer the input sizes, but this might not be
    #             accurate for complex models.
    #     """
    ...
    #         # Try to infer input sizes from the model's first layer
    #         try:
    #             module_list = list(model.modules())
    #             input_sizes = [list(module.in_features) for module in module_list[:len(model.modules()) // 2]]  # Assuming first half receives inputs
    #         except (AttributeError, IndexError):
    #             print("Warning: Could not infer input sizes from the model. Please provide the 'input_sizes' argument for accurate results.")
    #             input_sizes = None

    #     # Check if input_sizes is a list of tuples
    #     if not isinstance(input_sizes, list) or not all(isinstance(t, tuple) for t in input_sizes):
    #         raise ValueError("input_sizes must be a list of tuples representing input tensor sizes.")

    # # Print the model summary with detailed information
    #     summary(model, input_size=input_sizes)  # Pass input_sizes as a list of tuples


    # print_model_summary(latent_diffusion)
