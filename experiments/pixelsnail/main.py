...
parser_c.add_argument('--attn_nh', default=4, type=int, help='Number of attention heads.')
...
parser_c.add_argument('--attn_dv', default=64, type=int, help='Size of attention values.')
parser_c.add_argument('--attn_drop_rate', default=20, type=float, help='Dropout rate on attention logits.')
...
parser.add_argument('--image_dims', type=int, nargs='+', default=(3,32,32), help='Dimensions of the input data.')
...
def create_train_dataloader(args, train_dataset):
    ...
    # return train_dataloader
def lower_bits_transform(x, args):
   # Apply ToTensor first
   x = F.to_tensor(x)

   # Lower bits transformation
   x = x.mul(255).div(2**(8 - args.n_bits)).floor()

   # Apply preprocessing with n_bits argument
   x = preprocess(x, args.n_bits)

   return x
def create_conditional_target_transform(args):
   def target_transform_fn(y):
       return F.one_hot(y, num_classes=args.n_cond_classes)
   return target_transform_fn
# def create_abstract_transform(args, image_size):
#     # @dill.wraps  # Use dill.wraps to make the function picklable
#     def transform(img):
#         if min(img.size) < image_size:
#             img = F.resize(img, image_size)
#         elif max(img.size) >= 2 * image_size:
#             img = F.resize(img, 2 * image_size)
#         return T.Compose([
...
def fetch_dataloaders(args):
    ...
    transform = T.Compose([lower_bits_transform, preprocess])
    target_transform = create_conditional_target_transform(args)() if args.n_cond_classes else None
    
    if args.dataset=='abstract':
        image_size = (770, 1937)
        # image_size = 
        
        args.image_dims = (3, image_size[0], image_size[1])
        transform = T.Compose([
            # T.Lambda(lambda img: T.Resize(image_size)(img) if min(img.size) < image_size 
            #                 else (T.Resize(2*image_size)(img) if max(img.size) >= 2*image_size else img)),
            # T.Resize(2*image_size),
            T.RandomCrop(image_size),
            T.RandomHorizontalFlip(0.5),
            T.RandomVerticalFlip(0.5),
            T.ToTensor(),
        ...
        train_dataset = ImageFolder(r"C:\Users\Sukhvansh Jain\Desktop\IIT Study Material\Semester 4\CS-671-DL\PixelSNAIL code and model\Train" + "", transform=transform, target_transform=target_transform)
        valid_dataset = ImageFolder(r"C:\Users\Sukhvansh Jain\Desktop\IIT Study Material\Semester 4\CS-671-DL\PixelSNAIL code and model\DCASE_2023_Challenge_Task_7_Submission\AudioFiles\Submissions\A\TASys03\Train" + "", transform=transform, target_transform=target_transform)
    ...
    for epoch in range(args.start_epoch, args.start_epoch + args.n_epochs):
        ...
        if (epoch+1) % args.eval_interval == 0:
            ...
            torch.save({'epoch': epoch,
                        ...
                        os.path.join(args.output_dir, f'checkpoint_{epoch+1}_model2.pt'))
            torch.save(optimizer.state_dict(), os.path.join(args.output_dir, f'optim_checkpoint_{epoch+1}.pt'))
            if scheduler: torch.save(scheduler.state_dict(), os.path.join(args.output_dir, f'sched_checkpoint_{epoch+1}.pt'))
...
#    print(model)
    ...
    if args.restore_file:
        ...
        optimizer.load_state_dict(torch.load(r'C:\Users\Sukhvansh Jain\Desktop\IIT Study Material\Semester 4\CS-671-DL\PixelSNAIL code and model\results\pixelsnail\2024-03-17_12-28-52\optim_checkpoint_25.pt'))
        if scheduler:
            scheduler.load_state_dict(torch.load(r'C:\Users\Sukhvansh Jain\Desktop\IIT Study Material\Semester 4\CS-671-DL\PixelSNAIL code and model\results\pixelsnail\2024-03-17_12-28-52\sched_checkpoint_25.pt'))
...
# main.py --model pixelsnail --train --dataset your_custom_dataset --n_cond_classes your_number_of_classes --n_bits your_number_of_bits --image_dims your_image_dimensions --data_path /path/to/your/custom/dataset --lr your_learning_rate --batch_size  --n_epochs your_number_of_epochs


# main.py --train --dataset abstract --data_path D:/PYTHON Programming/Sound Scene Gen/archive (4)/A --batch_size 16 --lr 35e-5 pixelsnail --n_channels 160 --attn_dv 96
        

# D:\PYTHON Programming\Sound Scene Gen\DCASE_2023_Challenge_Task_7_Submission\AudioFiles\Submissions\A\TAsys02\
    
    
    
#python -u "C:\Users\Sukhvansh Jain\Desktop\IIT Study Material\Semester 4\CS-671-DL\PixelSNAIL code and model\main.py" --train --dataset abstract --data_path DCASE_2023_Challenge_Task_7_Submission\AudioFiles\Submissions\A\TASys02 --batch_size 1 --lr 35e-5 pixelsnail --n_channels 1 --attn_dv 1