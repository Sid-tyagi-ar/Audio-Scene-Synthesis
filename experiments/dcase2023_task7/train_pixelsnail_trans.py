...
# Import the PixelSNAILTransformer class
from pixelsnail_trans import PixelSNAILTransformer
...
def train(epoch, loader, model, optimizer, scheduler, device):
    ...
    for i, (bottom, class_id, salience, file_name) in enumerate(loader):
        ...
        # Process only the "bottom" input for the Transformer
        bottom = bottom.to(device)

        target = bottom
        out = model(bottom)  # No need for label_condition or salience_condition
...
if __name__ == '__main__':
    ...
    if args.hier == 'bottom':  # Example for bottom hierarchy
        # Instantiate the PixelSNAILTransformer
        model = PixelSNAILTransformer(
            shape=[20, 86],  # Image shape
            n_class=512,  # Number of classes
    ...
    for i in range(start_point, start_point + args.epoch):
        ...
        torch.save(
            ...
            f'checkpoint/pixelsnail-final/trans/{args.hier}_{str(i + 1).zfill(3)}.pt',
        )
