...
def get_dataset_filelist() -> list:
    ...
    for root_dir, _, file_list in os.walk("./DCASEFoleySoundSynthesisDevSet(1)"):
        for file_name in file_list:
            if os.path.splitext(file_name)[-1] == ".wav":
                class_name = root_dir.split('/')[-1].split("\\")[-1]
                if class_name in clas_dict:
                    training_files.append({
                    "class_id": root_dir.split("\\")[-1],
